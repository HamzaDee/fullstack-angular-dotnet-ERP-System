...
@Component({
  selector: 'app-items-transfer-voucher-list',
  templateUrl: './items-transfer-voucher-list.component.html',
  styleUrls: ['./items-transfer-voucher-list.component.scss']
})
export class ItemsTransferVoucherListComponent implements OnInit {
  ...
  (
    ...
    private transferStockService: TransferStkVoucherService,
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('ItemsTransferVoucherList');
  ...
  GetTransferStockVouchersList() {
    ...
    setTimeout(() => {
      this.transferStockService.GetTransferStockVoucherList().subscribe(result => {
        debugger
  ...
  DeleteTransferStockVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.transferStockService.DeleteTransferStockVoucher(id).subscribe((results) => {
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['TransferStockVoucher/ItemsTransferVoucherForm']);
  }

  AddTransferStockVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  OpenTransferStockVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  CopyTransferStockVoucher(id: any) {
    this.routePartsService.GuidToEdit = id;
  ...
  AttachmentTransferStockVoucher(id: any) {
    this.routePartsService.GuidToEdit = id;
    debugger      
  ...
  PostTransferStockVoucher(id: any) {
    Swal.fire({
      title: this.translateService.instant('AreYouSure?'),
      ...
      if (result.value) {
        this.transferStockService.PostTransferStockVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  refreshItemsTransferVoucherEnglish(data) {
    ...
    return {
      ...
      'Branch': x.branchName,
      'From Store': x.storeName,
      'To Store': x.toStoreName,
      'Total"': x.note,
      'Value': x.amount,
      'Status': x.statusName,
    }
  ...
  exportPdf() {
    ...
    if(currentLang == "ar"){
       head = [[' الحالة','القيمة',' ملاحظات',' الى مخزن','من مخزن',' الفرع',' تاريخ السند',' نوع السند',' رقم السند']]    }
    else{
       head = [['Status','Value','Total','To Store','From Store','Branch',' Voucher Date','Voucher Type','Voucher Number']]
    }
  ...
  let temp: (number | string)[] = [];
    ...
    let Title = currentLang == "ar" ? " قائمة سندات نقل المواد " : "Items Transfer Voucher List";
...
CalculateTotal() {
  if (this.tabelData) {
    ...
    return this.formatCurrency(Number(total.toFixed(3)), 3);
  }
}
