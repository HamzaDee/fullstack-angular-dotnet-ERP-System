...
@Component({
  selector: 'app-outputvoucherhlist',
  templateUrl: './outputvoucherhlist.component.html',
  styleUrl: './outputvoucherhlist.component.scss'
})
export class OutputvoucherhlistComponent implements OnInit {
      ...
      this.service.GetOutputVoucherList(this.voucherTypeEnum).subscribe(result => {
        ...
        if (this.childSearch) {
          ...
          if (result.length > 0) {
            ...
            this.childSearch.vprojectsList = result[0].invSearchCriteriaModel.projectsList;
            this.childSearch.vauthoritiesDonorList = result[0].invSearchCriteriaModel.authoritiesList;
          }
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['OutputVoucherH/OutputvoucherhForm']);
  }
  ...
  ApproveVoucher(id: any) {
    ...
    if (id != null && id != undefined && id != 0) {
      this.service.ApproveInvVoucher(id).subscribe((result) => {
        if (result.isSuccess == false && result.message === "msNoPermission") {
          this.alert.ShowAlert("msNoPermission", 'error');
          this.GetOutputVouchersList();
          return;
  ...
  exportPdf() {
    ...
    let Title = currentLang == "ar" ? " قائمة سندات الأخراج" : "Output Voucher List";
