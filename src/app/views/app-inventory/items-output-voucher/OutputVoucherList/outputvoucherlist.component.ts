...
@Component({
  selector: 'app-outputvoucherlist',
  templateUrl: './outputvoucherlist.component.html',
  styleUrls: ['./outputvoucherlist.component.scss']
})
export class OutputvoucherlistComponent implements OnInit {
  ...
  GetOutputVouchersList() {
    ...
    setTimeout(() => {
      this.outputService.GetOutputVoucherList(this.voucherTypeEnum).subscribe(result => {
        debugger
        ...
        if (this.childSearch) {
          ...
          if(result.length>0){
            ...
            this.useStoreInGrid = result[0].invSearchCriteriaModel.useStoreInGrid;       
          }
  ...
  PostOutputVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.outputService.PostInvVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['InventoryVouchers/OutputVoucherForm']);
  }
