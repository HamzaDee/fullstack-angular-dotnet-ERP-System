...
@Component({
  selector: 'app-items-reservation-voucher-list',
  templateUrl: './items-reservation-voucher-list.component.html',
  styleUrls: ['./items-reservation-voucher-list.component.scss']
})
export class ItemsReservationVoucherListComponent implements OnInit {
@ViewChild(InvSearchFormComponent)childSearch:InvSearchFormComponent;
  ...
  {
    this.SetTitlePage();
    this.GetItemsReservationVouchersList();
  ...
  GetItemsReservationVouchersList() {
    var currentLang = this.jwtAuth.getLang();
    const isArabic = currentLang === 'ar';      

    this.showLoader = true;
    setTimeout(() => {
      this.ItemsResService.GetItemsReservationVoucherList().subscribe(result => {
        ...
        if(currentLang == "ar"){
          this.refresItemsReservationVoucherArabic(this.tabelData);
         }
         else{
          this.refreshItemsReservationVoucherEnglish(this.tabelData);
         }
  ...
  DeleteItemsReservationVoucher(id: any) {
    Swal.fire({
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['ItemsReservationVoucher/ItemsReservationVoucherForm']);
  }

  AddItemsReservationVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  OpenItemsReservationVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  CopyItemsReservationVoucher(id: any) {
    this.routePartsService.GuidToEdit = id;
  ...
  AttachmentItemsReservationVoucher(id: any) {
    this.routePartsService.GuidToEdit = id;
    debugger      
  ...
  formatCurrency(value: number, decimalPlaces : number): string {    
    return this.ItemsResService.formatCurrency(value, decimalPlaces);
  }

  OpenOutputVoucherForm(reservedId: number) {
    debugger
    this.routePartsService.GuidToEdit = reservedId;
    
    // Construct the URL you want to navigate to
    const url = `/InventoryVouchers/OutputVoucherForm?reservedId=${reservedId}`;
  
    // Open the URL in a new tab
    window.open(url, '_blank');
  }

  OpenSalesInvoiceForm(reservedId: number) {
    debugger
    this.routePartsService.GuidToEdit = reservedId;
  
    // Construct the URL you want to navigate to
    const url = `/SalesInvoices/SalesInvoicesForm?reservedId=${reservedId}`;
  
    // Open the URL in a new tab
    window.open(url, '_blank');
  }

  refresItemsReservationVoucherArabic(data) {
    debugger
    this.data = data;
  ...
  refreshItemsReservationVoucherEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
   ...
   exportPdf() {
    ...
    if(currentLang == "ar"){
       head = [['الحالة',' ملاحظات','  الوقت','القيمة',' الفرع',' العميل',' تاريخ السند',' نوع السند',' رقم السند']]    }
    else{
       head = [['Status','Notes','Time','Value','Branch','Customer',' Voucher Date','Voucher Type','Voucher Number']]
    }
    ...
    pdf.setFontSize(14);
     ...
     let Title = currentLang == "ar" ?" قائمة سندات حجز مواد" : "Items Reservation Voucher List";
