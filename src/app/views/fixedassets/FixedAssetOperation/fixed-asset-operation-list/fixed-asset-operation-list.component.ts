...
@Component({
  ...
  selector: 'app-fixed-asset-operation-list',
  templateUrl: './fixed-asset-operation-list.component.html',
  styleUrls: ['./fixed-asset-operation-list.component.scss']
})
export class FixedAssetOperationListComponent implements OnInit {
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('FixedAssetOperationList');
  ...
  /*   GetAccVouchersHDList() {
      ...
      setTimeout(() => {
        this.FixedAssetOperationService.FixedAssetOperationList().subscribe(result => {
          debugger
  ...
  GetPaymentVoucherList() {
    ...
    this.showLoader = true;
    setTimeout(() => {
      this.FixedAssetOperationService.GetSuppPaymentVoucherList().subscribe(result => {
        ...
        if (currentLang == "ar") {
          this.refresOperationArabic(this.OperationList);
        }
        else {
          this.refreshOperationEnglish(this.OperationList);
        }
  ...
  ShowDetailsOnly(id) {
    ...
    this.router.navigate(['FixedAssetOperation/FixedAssetOperationForm']);
  }

  AddNewOperationInvoice(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.FixedAssetOperationService.PostAssetOperation(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  exportPdf() {
    ...
    if (currentLang == "ar") {
       head = [['الحالة', ' ملاحظات', 'المجموع', ' سعر الصرف', 'العمله', ' الفرع', ' تاريخ الحركة', ' رقم الحركة', ' نوع الحركة']]
    }
    else {
       head = [['Status', 'Notes', 'Total', 'Exchange Rate', 'Currency', 'Branch', ' Transaction Date', 'Transaction Number', 'Transaction Type']]
    }
    ...
    const Title = currentLang == "ar" ? " قائمة عمليات الاصول" : "Fixed Asset Operation";
  ...
  CalculateTotal() {
    if (this.OperationList) {
      let data = this.OperationList.filter(r => r.status != 67)
