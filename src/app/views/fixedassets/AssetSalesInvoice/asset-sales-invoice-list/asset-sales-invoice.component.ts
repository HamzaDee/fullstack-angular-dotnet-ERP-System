...
@Component({
  ...
  selector: 'app-asset-sales-invoice',
  templateUrl: './asset-sales-invoice.component.html',
  styleUrls: ['./asset-sales-invoice.component.scss']
})
export class AssetSalesInvoiceComponent implements OnInit {
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('AssetSalesInvoice');
...
/*   GetAccVouchersHDList() {
    ...
    setTimeout(() => {
      this.AsssestSalesInvoiceService.assetSalesInvoiceList().subscribe(result => {
        debugger
  ...
  GetPaymentVoucherList() {
    ...
    this.showLoader = true;
    setTimeout(() => {
      this.AsssestSalesInvoiceService.GetSuppPaymentVoucherList().subscribe(result => {
        debugger
        ...
        if(currentLang == "ar"){
          this.refresSalesInvoiceArabic(this.BillList);
         }
         else{
          this.refreshSalesInvoiceEnglish(this.BillList);
         }   
  ...
  ShowDetailsOnly(id){
    ...
    this.router.navigate(['AssetSalesInvoice/AssetSalesInvoiceForm']);
  }
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.AsssestSalesInvoiceService.PostServiceInvoice(id).subscribe((result) => {
          if (result.isSuccess) {
...
exportExcel() {
  ...
  saveAsExcelFile(buffer: any, fileName: string): void {
    ...
    if(currentLang == "ar"){
        head = [['الحالة',' ملاحظات','المجموع',' العميل',' سعر الصرف','العمله',' الفرع',' تاريخ الحركة',' رقم الحركة',' نوع الحركة']]    }
    else{
        head = [['Status','Notes','Total','Customer','Exchange Rate','Currency','Branch',' Transaction Date','Transaction Number','Transaction Type']]
    }
    ...
    temp[0]= part.transTypeName
     ...
     temp[9]= part.statusName
      ...
      const Title = currentLang == "ar" ? "قائمة فاتورة بيع أصل " : "Asset Sales Invoice";
