...
@Component({
  selector: 'app-returnpurinvoice-list',
  templateUrl: './returnpurinvoice-list.component.html',
  styleUrls: ['./returnpurinvoice-list.component.scss']
})
export class ReturnpurinvoiceListComponent implements OnInit {
  ...
  constructor
    (
      ...
      private returnInvoiceService: ReturnPurInvoiceService,
  ...
  ngOnInit(): void {
    this.SetTitlePage();
    this.GetReturnPurInvoiceList();
    this.getFavouriteStatus(this.screenId);
  }

  SetTitlePage() {
    this.TitlePage = this.translateService.instant('ReturnPurInvoiceList');
    this.title.setTitle(this.TitlePage);
  }

  GetReturnPurInvoiceList() {
    var currentLang = this.jwtAuth.getLang();
    const isArabic = currentLang === 'ar';      

    this.showLoader = true;
    setTimeout(() => {
      this.returnInvoiceService.GetReturnPurInvoiceList(this.voucherTypeEnum).subscribe(result => {
        debugger
        ...
        if(currentLang == "ar"){
          this.refresReturnpurinvoiceArabic(this.tabelData);
         }
         else{
          this.refreshReturnpurinvoiceEnglish(this.tabelData);
         }   
  ...
  PostReturnPurInvoice(Id, voucherTypeId, voucherNo) {
    Swal.fire({
      ...
      if (result.value) {
        debugger
        this.returnInvoiceService.PostPurchaseInvoice(Id, voucherTypeId, voucherNo).subscribe((result) => {
          debugger
  ...
  DeleteReturnPurInvoice(Id, voucherTypeId, voucherNo) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.returnInvoiceService.DeleteRetPurchaseInvoice(Id, voucherTypeId, voucherNo,this.voucherTypeEnum).subscribe((results) => {
          debugger
  ...
  ShowDetailsOnly(id: any) {
    ...
    this.router.navigate(['ReturnPurchaseInvoice/ReturnPurInvoiceForm']);
  }

  AddNewReturnPurInvoice(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  AttachmentReturnPurInvoice(id: any) {
    this.routePartsService.GuidToEdit = id;
    debugger
  ...
  refresReturnpurinvoiceArabic(data) {
    debugger
    this.data = data;
  ...
  refreshReturnpurinvoiceEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  exportPdf() {
    ...
    if(currentLang == "ar"){
       head = [['الحالة',' ملاحظات','المجموع',' سعر الصرف','العمله',' الفرع',' المورد',' تاريخ الفاتورة',' رقم الفاتورة',' نوع الفاتورة']]    }
     else{
       head = [['Status','Notes','Total','Exchange Rate','Currency','Branch','Supplier',' voucher Date','voucher Number','voucher Type']]
    }
    ...
    const Title = currentLang == "ar" ?"قائمة مرتجع فواتير الشراء" : "Return Purchase Invoice List";
