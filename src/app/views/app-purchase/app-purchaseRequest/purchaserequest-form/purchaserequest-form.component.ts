...
@Component({
  selector: 'app-purchaserequest-form',
  templateUrl: './purchaserequest-form.component.html',
  styleUrls: ['./purchaserequest-form.component.scss']
})
export class PurchaserequestFormComponent implements OnInit {
  ...
  headers = [
    ...
    { field: 'Bonus', label: this.translateService.instant('Bonus') },
  ];
  purExpensesList: any[] = [];
  expensesTypeList: any;
  accountsList: any;
  expCreditAcc: number;
  private skipInitLoad: boolean = false;
  ...
  ngOnInit(): void {
    debugger
    if (this.skipInitLoad) {
      this.skipInitLoad = false;
      return;
    }
    ...
    if (this.voucherId == null || this.voucherId == undefined || this.voucherId === "") {
      this.router.navigate(['PurchaseRequest/PurchaseRequestList']);
    }
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('PurchaseRequestForm');
  ...
  GetInitailEntryVoucher() {
    ...
    this.purService.GetInitailPurRequest(this.voucherId, this.opType).subscribe(result => {
      ...
      this.interPOReqList = result.internalPurchaseRequestList;
      ...
      if (this.costingDecimalPlaces == 0) {
        ...
        this.decimalPlaces = result.costingDecimalPlaces;
      }
      ...
      this.expCreditAcc = result.expCreditAcc;
      ...
      if (this.opType != 'createPO') {
        if (result.requestPurModelList !== undefined && result.requestPurModelList !== null) {
          ...
          this.accVouchersDTsList.forEach(element => {
            ...
            element.total = this.formatCurrency(element.total);
          })
          ...
          try {
            ...
            this.purService.GetUnitId(element.itemId).subscribe(
              (result) => {
                ...
                debugger
                this.PurcahseRequestAddForm.get("requestPurModelList").setValue(this.unAvlItems);
              },
              (error) => {
      ...
      source$.pipe(delay(0)).subscribe(() => {
        ...
        if (this.voucherId > 0) {
          ...
          this.PurcahseRequestAddForm.get("internalPRId").setValue(result.internalPRId);
          ...
          if (this.PurcahseRequestAddForm.value.currencyId == 0) {
            ...
            let currRate = this.currencyList.find(option => option.id === this.defaultCurrencyId)?.data1 || 0;
          ...
          this.PurcahseRequestAddForm.get("internalPRId").setValue(0);
  ...
  OnSaveForms() {
    ...
    for (let i = 0; i < this.accVouchersDTsList.length; i++) {
      ...
      if (element.price == undefined || element.price == null || element.price == "") {
        element.price = 0;
      }
      if (element.qty == undefined || element.qty == null || element.qty == "") {
        element.qty = 0;
      }
      if (element.bonus == undefined || element.bonus == null || element.bonus == "") {
        element.bonus = 0;
      }
    ...
    if (this.useStoreInGrid == false) {
      if (this.PurcahseRequestAddForm.value.storeId == 0 || this.PurcahseRequestAddForm.value.storeId == null || this.PurcahseRequestAddForm.value.storeId == undefined) {
        this.alert.ShowAlert("msgEnterAllData", 'error');
        stopExecution = true;
        this.disableSave = false;
        return false;
    ...
    if (this.purExpensesList.length > 0) {
      for (let i = 0; i < this.purExpensesList.length; i++) {
        ...
        if (element.affectCost == false && (element.debitAccId == 0 || element.debitAccId == null || element.debitAccId == undefined)) {
    ...
    if(!this.useStoreInGrid)
    {
      let store = this.PurcahseRequestAddForm.value.storeId;
      for (let i = 0; i < this.accVouchersDTsList.length; i++) {
        this.accVouchersDTsList[i].storeId = store;
      }
    }
    debugger
    ...
    this.PurcahseRequestAddForm.value.receiptExpensesModelList = this.purExpensesList;
  ...
  ClearAfterSave() {
    ...
    this.PurcahseRequestAddForm.get("purchaseExpensesModelList").setValue([]);    
    this.accVouchersDTsList = [];
  ...
  getCurrencyRate(event: any) {
    ...
    debugger
    this.accVouchersDTsList.forEach(element => {
      element.price = this.formatCurrency(Number(element.price))
      element.total = element.qty * Number(element.price);
      element.total = this.formatCurrency(element.total);
  ...
  AddNewLine() {
    ...
    if (!this.useStoreInGrid) {
      if (this.PurcahseRequestAddForm.value.storeId == 0) {
  ...
  GetVoucherTypeSetting(voucherTypeId: number) {
    ...
    if (this.opType == 'Add') {
      if (this.voucherStoreId > 0 && this.voucherStoreId != undefined && this.voucherStoreId != null) {
        ...
        this.PurcahseRequestAddForm.get("storeId").setValue(this.DefaultStoreId);
      }
      else {
        this.PurcahseRequestAddForm.get("storeId").setValue(0);
      }
  ...
  GetItemQtyLimit(row) {
    ...
    this.purService.GetItemQtyLimit(row.itemId, row.storeId).subscribe(res => {
      debugger
      if (res) {
  ...
  GetItemQty(row) {
    ...
    setTimeout(() => {
      this.purService.GetItemQty(row.itemId, row.storeId).subscribe(res => {
        debugger
        if (res.length == 0) {
          if ((row.qty) > this.remainQty && this.remainQty > 0) {
            this.showRemainQty = true;
            this.remainingQty = 0;
          ...
          if ((res[0].qty + row.qty) > this.remainQty && this.remainQty > 0) {
            this.showRemainQty = true;
  ...
  GetInternalPurchaseRequest(Id: number) {
    debugger
    if (Id > 0) {
      this.purService.GetInternalPurchaseRequest(Id).subscribe(res => {
        if (res) {
          this.PurcahseRequestAddForm.get("branchId")?.setValue(res.branchId ?? 0);
          this.PurcahseRequestAddForm.get("dealerId")?.setValue(res.dealerId ?? 0);
          this.PurcahseRequestAddForm.get("requestParty")?.setValue(res.requestParty ?? 0);
          this.PurcahseRequestAddForm.get("requestParty")?.setValue(res.requestParty ?? 0);
          this.PurcahseRequestAddForm.get("requestedBy")?.setValue(res.requestedBy ?? 0);
          this.PurcahseRequestAddForm.get("requestType")?.setValue(res.requestType ?? 0);
          this.PurcahseRequestAddForm.get("priority")?.setValue(res.priority ?? 0);
          // this.PurcahseRequestAddForm.patchValue(res);
          if (res.requestPurModelList !== undefined && res.requestPurModelList !== null) {
            let index = 0;
            this.accVouchersDTsList = res.requestPurModelList;
            this.accVouchersDTsList.forEach(element => {
              element.total = element.qty * element.price;
            })
            this.accVouchersDTsList.forEach(element => {
  ...
  exportHeadersToExcel() {
    ...
    XLSX.writeFile(wb, 'PurchaseRequestExcel.xlsx');
  }
  ...
  ImportFromExcel(event: any): void {
    ...
    reader.onload = (e: any) => {
      ...
      this.purService.ImportFromExcel(excelData).subscribe(
        (response) => {
          debugger
          if (response.length > 0) {
            ...
            this.accVouchersDTsList.forEach((row) => {
              const qty = +row.qty || 0;
              const price = +row.price || 0;
              row.total = qty * price;
            });

            const requests = this.accVouchersDTsList.map((Row, i) =>
              this.purService.GetItemUintbyItemId(Row.itemId).pipe(
                tap(res => {
                  debugger
                  this.unitsList[i] = res;
                  ...
                  if (res.length === 2) {
                    ...
                    const unit = res.find(r => r.id == Row.unitId);
                    this.accVouchersDTsList[i].unitId = unit ? Row.unitId : 0;
                  } else {
                    this.accVouchersDTsList[i].unitId = res.length > 0 ? res[0].id : 0;
                  }

                  this.bounsunitsList[i] = res;

                  if (response[i].bonus > 0) {
                    if (res.length === 2) {
                      this.accVouchersDTsList[i].bonusUnitId = res[1].id;
                    } else if (this.opType === "Edit") {
                      const unit = res.find(r => r.id == Row.unitId);
                      this.accVouchersDTsList[i].bonusUnitId = unit ? Row.unitId : 0;
                    } else {
                      this.accVouchersDTsList[i].bonusUnitId = res.length > 0 ? res[0].id : 0;
                    }
                  }
    ...
    this.purExpensesList.push(
      {
        ...
        amount: "",
        debitAccId: 0,
        creditAccId: this.expCreditAcc,
        note: "",
        affectCost: true,
        index: "",
      });
    this.PurcahseRequestAddForm.get("purchaseExpensesModelList").setValue(this.purExpensesList);
  }
