...
@Component({
  ...
  templateUrl: './supplierpaymentvoucher-list.component.html',
...
export class SupplierpaymentvoucherListComponent implements OnInit {
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('SupppaymentvoucherList');
  ...
  GetPaymentVoucherList() {
    ...
    setTimeout(() => {
      this.supPaymentvoucherService.GetSuppPaymentVoucherList().subscribe(result => {
        ...
        if (currentLang == "ar") {
          this.refreshSupplierpaymentvoucherListArabic(this.tabelData);
        }
        else {
          this.refreshSupplierpaymentvoucherListEnglish(this.tabelData);
        }
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.supPaymentvoucherService.PostSuppPaymentVoucher(id).subscribe((result) => {
          debugger
          if (result.isSuccess) {
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['SupplierPaymentVoucher/SupppaymentvoucherForm']);
  }
  ...
  AddSupplierPaymentVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  refreshSupplierpaymentvoucherListArabic(data) {
    ...
    const formattedDate = new Date(x.voucherDate).toLocaleDateString('ar-EG');
      ...
      'المجموع': x.amount,
      'ملاحظات': x.note,
      'طريقة الصرف': x.paymentMethods.join(', '),
      'الحالة': x.statusName,
      }
    });
  }
  ...
  refreshSupplierpaymentvoucherListEnglish(data) {
    ...
    return {
      ...
      'Total': x.amount,
      'Notes': x.note,
      'Exchange Method': x.paymentMethods.join(', '),
      'status': x.statusName,
    }
    });
  }
  ...
  exportPdf() {
    ...
    if (currentLang == "ar") {
       head = [['الحالة', 'طريقة الصرف', 'ملاحظات', ' المجموع', 'سعر الصرف', '  العملة', ' المورد ', ' الفرع ', ' تاريخ السند', '  رقم السند', ' نوع السند']]
    }
    else {
       head = [['status', 'Exchange Method ', 'Notes', 'Total', ' Exchange Rate', 'Currency', 'Supplier ', 'Branch ', 'Voucher Date ', ' Voucher Number', 'Voucher Type']]
    }
    ...
    const Title = currentLang == "ar" ? "قائمة سندات صرف موردين" : "Supplier Payment Voucher List";
