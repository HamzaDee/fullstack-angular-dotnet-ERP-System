...
@Component({
  selector: 'app-suppopeningbalance-list',
  templateUrl: './suppopeningbalance-list.component.html',
  styleUrls: ['./suppopeningbalance-list.component.scss']
})
export class SuppopeningbalanceListComponent implements OnInit {
  ...
  constructor(private title: Title,
    ...
    private suppService: AppPayablesService,
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('SuppOpeningBalanceList');
  ...
  GetSuppOpeningBalanceList() {
    ...
    setTimeout(() => {
      this.suppService.GetSuppliersOpeningBalanceList().subscribe(result => {
        ...
        if (currentLang == "ar") {
          this.refresSuppopeningbalanceListArabic(this.tabelData);
        }
        else {
          this.refreshSuppopeningbalanceListEnglish(this.tabelData);
        }
  ...
  PostOpeningBalance(id: any, isPosted: boolean) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.suppService.PostOpeningBalance(id, isPosted).subscribe((result) => {
          if (result) {
            if (result.isSuccess == false && result.message == "msNoPermission") {
              this.alert.ShowAlert("msNoPermission", 'error');
              return;
  ...
  DeleteOpeningBalance(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.suppService.DeleteOpeningBalance(id).subscribe((results) => {
          if (results) {
  ...
  OpenOpeningBalanceForm(id: any) {
    ...
    this.router.navigate(['SuppliersOpeningBalance/SuppopeningbalanceForm']);
  }

  AddOpeningBalanceForm(id: any) {
    debugger
    this.routePartsService.GuidToEdit = id
  ...
  CopyOpeningBalance(id: any) {
    ...
    this.router.navigate(['Suppopeningbalance/SuppopeningbalanceForm']);
  }
  ...
  AttachmentOpeningBalance(id: any) {
    this.routePartsService.GuidToEdit = id;
    debugger
  ...
  refresSuppopeningbalanceListArabic(data) {
    debugger
    this.data = data;
  ...
  refreshSuppopeningbalanceListEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  exportPdf() {
    ...
    if (currentLang == "ar") {
       head = [['الحالة', ' معامل التحوبل', 'العملة', ' الفرع', 'تاريخ السند', 'رقم السند', 'نوع السند']]
    }
    else {
       head = [['Status', 'Converion Factor', 'Currency', 'Branch', ' Voucher Date', 'Voucher Number', 'Voucher Type']]
    }
    ...
    const Title = currentLang == "ar" ? "قائمة الأرصدة الأفتتاحية للموردين": "Supplier Opening Balance List";
