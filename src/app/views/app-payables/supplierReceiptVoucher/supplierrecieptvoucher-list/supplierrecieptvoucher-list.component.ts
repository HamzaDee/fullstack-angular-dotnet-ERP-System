...
@Component({
  selector: 'app-supplierrecieptvoucher-list',
  templateUrl: './supplierrecieptvoucher-list.component.html',
  styleUrls: ['./supplierrecieptvoucher-list.component.scss']
})
export class SupplierrecieptvoucherListComponent implements OnInit {
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('SuppReceiptvoucherList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces: number): string {
    debugger
    return this.supReceiptvoucherService.formatCurrency(value, decimalPlaces);
  }
  ...
  GetPaymentVoucherList() {
    ...
    setTimeout(() => {
      this.supReceiptvoucherService.GetSuppReceiptVoucherList().subscribe(result => {
        ...
        if (currentLang == "ar") {
          this.refresSupplierrecieptvoucherArabic(this.tabelData);
        }
        else {
          this.refreshSupplierrecieptvoucherEnglish(this.tabelData);
        }
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.supReceiptvoucherService.PostSuppReceiptVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['SupplierReceiptVoucher/SuppReceiptvoucherForm']);
  }
  ...
  AddSupplierReceiptVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  refresSupplierrecieptvoucherArabic(data) {
    debugger
    this.data = data;
  ...
  refreshSupplierrecieptvoucherEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  exportPdf() {
    ...
    const Title = currentLang == "ar" ? "قائمة سندات قبض الموردين  " : "Supplier Receipt Voucher List";
