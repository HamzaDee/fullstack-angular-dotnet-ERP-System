...
@Component({
  selector: 'app-supplierdebitvoucher-list',
  templateUrl: './supplierdebitvoucher-list.component.html',
  styleUrls: ['./supplierdebitvoucher-list.component.scss']
})
export class SupplierdebitvoucherListComponent implements OnInit {
  ...
  {
      this.SetTitlePage();
      this.GetDebitNoteVoucherList();
      this.getFavouriteStatus(this.screenId);
  }

  SetTitlePage() {
    this.TitlePage = this.translateService.instant('SuppDebitVoucherList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces : number): string {
    debugger
    return this.supDebitvoucherService.formatCurrency(value, decimalPlaces);
  } 
  ...
  GetDebitNoteVoucherList() {
    ...
    setTimeout(() => {
      this.supDebitvoucherService.GetSuppDebitNoteVoucherList().subscribe(result => {
        ...
        if(currentLang == "ar"){
          this.refresSupplierdebitvoucherArabic(this.tabelData);
         }
         else{
          this.refreshSupplierdebitvoucherEnglish(this.tabelData);
         }  
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.supDebitvoucherService.PostSuppDebitNoteVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['SupplierDepitNoteVoucher/SuppDebitVoucherForm']);
  }
  ...
  AddSupplierDebitNoteVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  refresSupplierdebitvoucherArabic(data) {
    debugger
    this.data = data;
  ...
  refreshSupplierdebitvoucherEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  if(currentLang == "ar"){
     head = [['الحالة','طريقة القبض',' ملاحظات','المجموع',' سعر الصرف','العمله',' المورد',' الفرع',' تاريخ السند',' رقم السند',' نوع السند']]    }
  else{
     head = [['Status','Payment Method','Notes','Total','Exchange Rate','Currency','Supplier','Branch',' Voucher Date','Voucher Number','Voucher Type']]
  }
  ...
  let temp: (number|string)[] = [];
    ...
    const Title = currentLang == "ar" ? "قائمة اشعارات مدين موردين": "Supplier Debit Voucher List ";
