...
@Component({
  selector: 'app-customerrecieptvoucher-list',
  templateUrl: './customerrecieptvoucher-list.component.html',
  styleUrls: ['./customerrecieptvoucher-list.component.scss']
})
export class CustomerrecieptvoucherListComponent implements OnInit {
  ...
  ngOnInit(): void 
  {
    ...
    if (!token) {
      ...
      // تسجيل دخول تلقائي أول مرة
      this.jwtAuth.signin(this.username, this.password, 1).subscribe({
        next: (res) => {
      ...
      // Already logged in, just load voucher
      this.SetTitlePage();
      this.GetCustRecieptVoucherList();
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('CustRecieptvoucherList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces : number): string {
    
    return this.cusReceiptvoucherService.formatCurrency(value, decimalPlaces);
  } 

  GetCustRecieptVoucherList() {
    var currentLang = this.jwtAuth.getLang();
    const isArabic = currentLang === 'ar';      

    this.showLoader = true;
    setTimeout(() => {
      this.cusReceiptvoucherService.GetCustReceiptVoucherList().subscribe(result => {
        ...
        if(currentLang == "ar"){
          this.refresCustomerrecieptvoucherArabic(this.tabelData);
         }
         else{
          this.refreshCustomerrecieptvoucherEnglish(this.tabelData);
         } 
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.cusReceiptvoucherService.PostCustReceiptVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
        ...
        this.cusReceiptvoucherService.DeleteCustReceiptVoucher(id).subscribe((result) => {
          ...
          if (result.isSuccess == true) {
            ...
            if(result.isSuccess == false && result.message =="msNoPermission")
              ...
              {
                if(result.message == "msgCantDeleteVoucher"){

                  this.alert.ShowAlert("msgCantDeleteVoucher",'error')
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['CustomerReceiptVoucher/CustRecieptvoucherForm']);
  }
  ...
  AddCuxtomerReceiptVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  refresCustomerrecieptvoucherArabic(data) {
    debugger
    this.data = data;
  ...
  refreshCustomerrecieptvoucherEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  exportPdf() {
    ...
    if(currentLang == "ar"){
      head = [['الحالة',' طريقة القبض',' ملاحظات','المجموع',' سعر الصرف','العمله',' العميل',' الفرع',' تاريخ السند',' رقم السند',' نوع السند']]    }
    else{
  ...
  let temp: (number | string)[] = [];
    ...
    temp[7]= part.amount
    temp[8]= part.note 
    temp[9]= part.paymentMethods.join(', '),
    ...
    const Title = currentLang == "ar" ?"قائمة سندات قبض العملاء " : "Customer Reciept Voucher List";
  ...
  PrintCustomerReciptvoucher(voucherId: number,Balance) {
    debugger
