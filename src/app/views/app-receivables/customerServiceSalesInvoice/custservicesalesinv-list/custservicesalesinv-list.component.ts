...
@Component({
  selector: 'app-custservicesalesinv-list',
  templateUrl: './custservicesalesinv-list.component.html',
  styleUrls: ['./custservicesalesinv-list.component.scss']
})
export class CustservicesalesinvListComponent implements OnInit {
  ...
  constructor
            (
              ...
              private invoiceSalesService: serviceInvoiceSalesService,
  ...
  {
    this.SetTitlePage();
    this.GetSalesServiceInvoiceList();
    this.getFavouriteStatus(this.screenId);
  }


  SetTitlePage() {
    this.TitlePage = this.translateService.instant('CustServiceSalesInvoiceList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces : number): string {
    return this.invoiceSalesService.formatCurrency(value, decimalPlaces);
  } 

  GetSalesServiceInvoiceList() {
    var currentLang = this.jwtAuth.getLang();
    const isArabic = currentLang === 'ar';      

    this.showLoader = true;
    setTimeout(() => {
      this.invoiceSalesService.GetServiceInvoiceSalesList().subscribe(result => {
        debugger
        ...
        if(currentLang == "ar"){
          this.refresCustservicesalesinvArabic(this.tabelData);
         }
         else{
          this.refreshCustservicesalesinvEnglish(this.tabelData);
         }   
  ...
  PostSalesServiceInvoice(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.invoiceSalesService.PostServicesSalesInv(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  DeleteSalesServiceInvoice(id: any) {
    Swal.fire({
      title: this.translateService.instant('AreYouSure?'),
      ...
      if (result.value) {
        this.invoiceSalesService.DeleteServicesSalesInv(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  OpenSalesServiceInvoiceForm(id: any) {
    
    this.routePartsService.GuidToEdit = id
    this.routePartsService.Guid2ToEdit = 'Edit';
    this.router.navigate(['ServicesSalesInv/CustServiceSalesInvoiceForm']);
  }
  
  AddCustomerSalesServiceInvoiceForm(id: any) {
  ...
  CopySalesOpeningBalance(id: any) {
    this.routePartsService.GuidToEdit = id;
    this.routePartsService.Guid2ToEdit = 'Copy';
    this.router.navigate(['ServicesSalesInv/CustServiceSalesInvoiceForm']);
  }

  ReverseSalesServiceInvoice(id: any) {
    this.routePartsService.GuidToEdit = id;
  ...
  CopySalesServiceInvoice(id: any) {
    this.routePartsService.GuidToEdit = id;
    this.routePartsService.Guid2ToEdit = 'Copy';
    this.router.navigate(['ServicesSalesInv/CustServiceSalesInvoiceForm']);
  }

  refresCustservicesalesinvArabic(data) {
    debugger
    this.data = data;
  ...
  refreshCustservicesalesinvEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  exportPdf() {
    ...
    if(currentLang == "ar"){
       head = [['الحالة',' ملاحظات','المجموع',' سعر الصرف','العمله',' الفرع',' المندوب',' العميل',' تاريخ الفاتورة',' رقم الفاتورة',' نوع الفاتورة']]    }
    else{
       head = [['Status','Notes','Total','Exchange Rate','Currency','Branch','Man','Customer',' Voucher Date','Voucher Number','Voucher Type']]
    }
    ...
    const Title = currentLang == "ar" ? "قائمة فواتير بيع خدمات" : "Sales Service Invoice List";
