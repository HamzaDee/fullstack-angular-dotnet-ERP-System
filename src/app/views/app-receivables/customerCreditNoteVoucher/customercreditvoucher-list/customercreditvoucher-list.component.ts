...
@Component({
  selector: 'app-customercreditvoucher-list',
  templateUrl: './customercreditvoucher-list.component.html',
  styleUrls: ['./customercreditvoucher-list.component.scss']
})
export class CustomercreditvoucherListComponent implements OnInit {
  ...
  {
      this.SetTitlePage();
      this.GetCreditNoteVoucherList();
      this.getFavouriteStatus(this.screenId);
  }


  SetTitlePage() {
    this.TitlePage = this.translateService.instant('CustCreditvoucherList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces : number): string {
    return this.cusCreditvoucherService.formatCurrency(value, decimalPlaces);
  } 
  ...
  GetCreditNoteVoucherList() {
    ...
    setTimeout(() => {
      this.cusCreditvoucherService.GetCustCreditNoteVoucherList().subscribe(result => {
        ...
        if(currentLang == "ar"){
          this.refresCustomerdCreditvoucherArabic(this.tabelData);
         }
         else{
          this.refreshCustomerCreditvoucherEnglish(this.tabelData);
         }  
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.cusCreditvoucherService.PostCustCreditNoteVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['CustomersCreditNote/CustCreditvoucherForm']);
  }
  ...
  AddCutomerCreditNoteVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  refresCustomerdCreditvoucherArabic(data) {
    debugger
    this.data = data;
    ...
    const formattedDate = new Date(x.voucherDate).toLocaleDateString('ar-EG');
      ...
      'المجموع': x.amount,
      'ملاحظات': x.note,
      'طريقة القبض': x.paymentMethods[0],
      'الحالة': x.statusName,
    }
    });
  }

  refreshCustomerCreditvoucherEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
    const formattedDate = new Date(x.voucherDate).toLocaleDateString('en-GB');
      ...
      'Total': x.amount,
      'Notes': x.note,
      'Payment Method': x.paymentMethods[0],
      'Status': x.statusName,
    }
    });
  }
  ...
  if(currentLang == "ar"){
     head = [['الحالة',' طريقه القبض',' ملاحظات','المجموع',' سعر الصرف','العمله',' العميل',' الفرع',' تاريخ السند',' رقم السند',' نوع السند']]    }
  else{
     head = [['Status','Payment Method','Notes','Total','Exchange Rate','Currency','Customer','Branch',' Voucher Date','Voucher Number','Voucher Type']]
  }
  ...
  let temp: (number | string)[] = [];
    ...
    temp[4]= part.custName 
    temp[5]= part.currencyName
    temp[6]= part.currRate
    temp[7]= part.amount
    temp[8]= part.note 
    temp[9]= part.paymentMethods[0]
    ...
    const Title = currentLang == "ar" ? "قائمة إشعارات دائن عملاء" : "Customer Credit  Voucher List";
