...
@Component({
  selector: 'app-customerdebitvoucher-list',
  templateUrl: './customerdebitvoucher-list.component.html',
  styleUrls: ['./customerdebitvoucher-list.component.scss']
})
export class CustomerdebitvoucherListComponent implements OnInit {
  ...
  constructor
              (
                ...
                private cusDebitvoucherService: CustDebitNoteVoucherService,
  ...
  {
      this.SetTitlePage();
      this.GetCustPaymentVoucherList();
      this.getFavouriteStatus(this.screenId);
  }


  SetTitlePage() {
    this.TitlePage = this.translateService.instant('CustDebitvoucherList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces : number): string {
    return this.cusDebitvoucherService.formatCurrency(value, decimalPlaces);
  } 
  ...
  GetCustPaymentVoucherList() {
    ...
    setTimeout(() => {
      this.cusDebitvoucherService.GetCustDebitNoteVoucherList().subscribe(result => {
        ...
        if(currentLang == "ar"){
          this.refresCustomerdebitvoucherArabic(this.tabelData);
         }
         else{
          this.refreshCustomerdebitvoucherEnglish(this.tabelData);
         }   
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.cusDebitvoucherService.PostCustDebitNoteVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  DeleteVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.cusDebitvoucherService.DeleteCustDebitNoteVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  OpenDetailsForm(id: any) {
    ...
    this.router.navigate(['CustomersDebitNote/CustDebitvoucherForm']);
  }
  ...
  AddCustomerDebitNoteVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  refresCustomerdebitvoucherArabic(data) {
    debugger
    this.data = data;
  ...
  refreshCustomerdebitvoucherEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  totalAmount = filteredData.reduce((sum, part) => sum + part.amount * part.currRate, 0);
    ...
    const Title = currentLang == "ar" ? "قائمة اشعارات مدين عملاء " :"Customer Debit Voucher List";
