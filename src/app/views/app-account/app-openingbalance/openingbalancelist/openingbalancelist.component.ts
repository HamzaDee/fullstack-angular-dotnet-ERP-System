...
@Component({
  selector: 'app-openingbalancelist',
  templateUrl: './openingbalancelist.component.html',
  styleUrls: ['./openingbalancelist.component.scss']
})
export class OpeningbalancelistComponent implements OnInit {
  ...
  constructor(
    ...
    private AppOpeningBalanceService: AppOpeningBalanceService,
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('OpeningBalanceList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces: number): string {
    return this.AppOpeningBalanceService.formatCurrency(value, decimalPlaces);
  }
  ...
  GetOpeningBalanceList() {
    ...
    setTimeout(() => {
      this.AppOpeningBalanceService.GetOpeningBalanceList().subscribe(result => {
        ...
        if (currentLang == "ar") {
          this.refresOpeningbalanceArabic(this.tabelData);
        }
        else {
          this.refreshOpeningbalanceEnglish(this.tabelData);
        }
  ...
  PostOpeningBalance(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.AppOpeningBalanceService.PostOpeningBalance(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  DeleteOpeningBalance(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.AppOpeningBalanceService.DeleteOpeningBalance(id).subscribe((results) => {
          if (results) {
  ...
  AddOpeningBalanceForm(id: any) {
    ...
    this.router.navigate(['OpeningBalance/OpeningbalanceForm']);
  }
  ...
  refresOpeningbalanceArabic(data) {
    debugger
    this.data = data;
  ...
  refreshOpeningbalanceEnglish(data) {
    debugger
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  exportPdf() {
    ...
    const head = currentLang === "ar"
      ...
      : [['Status', 'Notes', 'Total', 'Exchange Rate', 'Currency', 'Branch', 'Voucher Date', 'Voucher Number', 'Voucher Type']];
    ...
    if (currentLang == "ar") {
      footRow[5] = "المجموع";
      footRow[6] = totalAmount.toFixed(2); foot = [footRow.reverse()];
    }
    else {
      footRow[5] = "Total";
      footRow[6] = totalAmount.toFixed(2); foot = [footRow.reverse()];
    ...
    const title = currentLang === "ar"
      ? "قائمة الأرصدة الافتتاحية للحسابات"
      : "Accounts Opening Balance List";
