...
@Component({
  selector: 'app-servicereturn',
  templateUrl: './servicereturn.component.html',
  styleUrls: ['./servicereturn.component.scss']
})
...
/*   statusList: { id: number; text: string }[] = [
  ...
  ngOnInit(): void {
    this.SetTitlePage();
    this.GetReturnServiceInvoiceForm();
    this.GetReturnServiceInvoiceInitialForm();
    this.getFavouriteStatus(this.screenId);
  }

  GetReturnServiceInvoiceForm() {
    debugger
  ...
  GetReturnServiceInvoiceInitialForm() {
    debugger
    this.ReportsService.GetServicesReturnInvoicesReportModel().subscribe((result) => {
      debugger
  ...
  GetReport() {
    ...
    setTimeout(() => {
     ...
     // formValues.voucherStatus = this.selectedstatus;
      ...
      this.ReportsService.GetReturnServiceInvoices(
        formValues.fromdate,
        formValues.todate,
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('ReturnService');
  ...
  exportExcel1() {
    import("xlsx").then(xlsx => {
      ...
      // حساب المجاميع
      const total = this.voucherData.reduce((sum, item) => sum + parseFloat(item.voucherAmount || 0), 0);
  ...
  exportPdf1() {
    ...
    this.voucherData.forEach(function (part) {
      ...
      total += parseFloat(part.voucherAmount) || 0;
    ...
    if (currentLang === "ar") {
      footRow[9] = "المجموع";
      footRow[10] = total.toFixed(2);     // مدين
