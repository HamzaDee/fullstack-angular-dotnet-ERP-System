...
@Component({
  selector: 'app-servicereturn-list',
  templateUrl: './servicereturn-list.component.html',
  styleUrls: ['./servicereturn-list.component.scss']
})
export class ServicereturnListComponent implements OnInit {
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('ReturnServiceInvoiceList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces: number): string {
    return this.services.formatCurrency(value, decimalPlaces);
  }
  ...
  GetServiceInvoiceList() {
    ...
    setTimeout(() => {
      this.services.GetServiceInvoiceList().subscribe(result => {
        debugger
  ...
  PostServiceInvoice(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.services.PostServiceInvoice(id).subscribe((result) => {
          if (result.isSuccess) {
            ...
            if (result.message) {
              this.alert.ShowAlert("msgTaxError", 'error');
            }
            else{
  ...
  OpenServiceInvoiceForm(id: any) {
    ...
    this.router.navigate(['ReturnServiceInvoice/ReturnServiceInvoiceForm']);
  }
