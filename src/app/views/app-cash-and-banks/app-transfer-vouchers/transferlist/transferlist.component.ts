...
export class TransferlistComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.SetTitlePage();
    this.GetTransferVoucherList();
    this.getFavouriteStatus(this.screenId);
  }

  SetTitlePage() {
    this.TitlePage = this.translateService.instant('TransferVoucherList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces: number): string {
    return this.transferVoucherService.formatCurrency(value, decimalPlaces);
  }

  GetTransferVoucherList() {
    ...
    setTimeout(() => {
      this.transferVoucherService.GetTransferVoucherList().subscribe(result => {
        ...
        if (currentLang == "ar") {
          this.refreshTransferlistTableArabic(this.tabelData);
        }
        else {
          this.refreshTransferlistTableEnglish(this.tabelData);
        }
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.transferVoucherService.PostTransferVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  OpenTransferVoucherForm(id: any) {
    debugger
    this.routePartsService.GuidToEdit = id;
    this.routePartsService.Guid2ToEdit = 'Show';
    this.routePartsService.Guid3ToEdit = true;
    this.router.navigate(['TransferVoucher/TransferVoucherform']);
  }

  AddTransferVoucherForm(id: any) {
    debugger
    this.routePartsService.GuidToEdit = id
  ...
  EditTransferVoucherForm(id: any) {
    debugger
    this.routePartsService.GuidToEdit = id
  ...
  exportPdf() {
    ...
    const title = currentLang === "ar"
      ? "كشف قائمة سندات التحويل والأيداع النقدي"
      : "Transfer voucher and cash deposit";
  ...
  refreshTransferlistTableArabic(data) {
  ...
  refreshTransferlistTableEnglish(data) {
    this.exportData = data;
    this.exportData = this.tabelData.map(x => {
