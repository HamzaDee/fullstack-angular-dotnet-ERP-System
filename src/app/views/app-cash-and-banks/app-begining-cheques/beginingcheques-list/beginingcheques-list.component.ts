...
@Component({
  selector: 'app-beginingcheques-list',
  templateUrl: './beginingcheques-list.component.html',
  styleUrls: ['./beginingcheques-list.component.scss']
})
export class BeginingchequesListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.SetTitlePage();
    this.GetBeginingchequesList();
    this.getFavouriteStatus(this.screenId);
  }

  SetTitlePage() {
    this.TitlePage = this.translateService.instant('BeginingchequesList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces: number): string {
    return this.beginingChequesService.formatCurrency(value, decimalPlaces);
  }

  GetBeginingchequesList() {
    var currentLang = this.jwtAuth.getLang();
    const isArabic = currentLang === 'ar';

    this.showLoader = true;
    setTimeout(() => {
      this.beginingChequesService.GetBeginingChequesList().subscribe(result => {
        ...
        if (currentLang == "ar") {
          this.refresBeginingchequesListArabic(this.tabelData);
        }
        else {
          this.refreshBeginingchequesListEnglish(this.tabelData);
        }
  ...
  PostBeginingcheques(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.beginingChequesService.PostBeginingCheques(id).subscribe((result) => {
          if (result.isSuccess) {
      ...
      if (result.value) {
        this.beginingChequesService.DeleteBeginingCheques(id).subscribe((results) => {
          if (results) {
            ...
            else if (results.isSuccess == false && results.message == "msgCantDeleteVoucher") {
  ...
  OpenBeginingchequesForm(id: any) {
    this.routePartsService.GuidToEdit = id
    this.routePartsService.Guid2ToEdit = 'Show';
    this.routePartsService.Guid3ToEdit = true;
    this.router.navigate(['BeginningCheques/Beginingchequesform']);
  }

  EditBeginingchequesForm(id: any) {
  ...
  AttachmentBeginingcheques(id: any) {
    this.routePartsService.GuidToEdit = id;
  ...
  refresBeginingchequesListArabic(data) {
    ...
    this.exportData = this.data.map(x => {
      ...
      return {
        ...
        'المجموع': x.amount,
        'ملاحظات': x.note,
        'طريقة الدفع': x.branchName,
        'الحالة': x.statusName,
      }
    });
  }

  refreshBeginingchequesListEnglish(data) {
    this.data = data;
    this.exportData = this.data.map(x => {
  ...
  exportPdf() {
    ...
    if (currentLang == "ar") {
       head = [['الحالة', ' طريقة الدفع', ' ملاحظات', 'المجموع', ' سعر الصرف', 'العملة', ' الفرع', ' تاريخ السند', ' رقم السند', ' نوع السند']]
    }
    else {
    ...
    const title = currentLang === "ar"
      ? "قائمة شيكات اول المدة"
      : "Begining Cheques List";
