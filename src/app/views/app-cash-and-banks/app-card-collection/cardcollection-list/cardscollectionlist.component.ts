...
@Component({
  selector: 'app-cardscollectionlist',
  templateUrl: './cardscollectionlist.component.html',
  styleUrl: './cardscollectionlist.component.scss'
})
export class CardscollectionlistComponent implements OnInit {
  ...
  constructor(
   ...
   ngOnInit(): void {
    ...
    SetTitlePage() {
      this.TitlePage = this.translateService.instant('Cardscollectionlist');
      this.title.setTitle(this.TitlePage);
    }
  
    formatCurrency(value: number, decimalPlaces: number): string {
      return this.service.formatCurrency(value, decimalPlaces);
    }
    ...
    const isArabic = currentLang === 'ar';
      ...
      setTimeout(() => {
        debugger
        this.service.GetCreditCardCollectionList().subscribe(result => {
          debugger
          ...
          if (currentLang == "ar") {
            this.refreshCardscollectionListReportTableArabic(this.tabelData);
          }
          else {
            this.refreshCardscollectionListReportTableEnglish(this.tabelData);
          }
    ...
    PostEntryVoucher(id: any) {
      Swal.fire({
        ...
        if (result.value) {
          this.service.PostCreditCard(id).subscribe((result) => {
            if (result.isSuccess) {
    ...
    OpenDetailsForm(id) {
      ...
      this.router.navigate(['CreditCardsCollection/Cardscollectionform']);
    }
    ...
    refreshCardscollectionListReportTableArabic(data) {
    ...
    refreshCardscollectionListReportTableEnglish(data) {
      this.exportData = data;
      this.exportData = this.tabelData.map(x => {
    ...
    const isArabic = currentLang === 'ar';
      ...
      if (currentLang == "ar") {
         head = [['الحالة', 'المجموع', 'سعر الصرف', 'العملة', 'الفرع ', 'تاريخ السند', 'رقم السند', 'نوع السند']]
      }
      else {
         head = [['Status', 'Total', 'Exchange Rate', 'Currency', 'Branch', 'Voucher Date', 'Voucher No', 'Voucher Type']]
      }
    ...
    PrintCardscollection(voucherId: number) {
      debugger
