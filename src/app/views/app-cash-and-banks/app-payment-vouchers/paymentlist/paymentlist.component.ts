...
export class PaymentlistComponent implements OnInit {
  ...
  SetTitlePage() {
    this.TitlePage = this.translateService.instant('PaymentVoucherList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces: number): string {

    return this.PaymentvoucherService.formatCurrency(value, decimalPlaces);
  }
  ...
  GetPaymentVoucherList() {
    ...
    setTimeout(() => {
      this.PaymentvoucherService.GetPaymentVoucherList().subscribe(result => {
        ...
        if (currentLang == "ar") {
          this.refresPaymentlistArabic(this.tabelData);
        }
        else {
          this.refreshPaymentlistEnglish(this.tabelData);
        }
  ...
  PostEntryVoucher(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.PaymentvoucherService.PostPaymentVoucher(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  OpenEntryVoucherForm(id: any) {
    ...
    this.router.navigate(['PaymentVoucher/Paymentform']);
  }

  AddPaymentVoucherForm(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  refreshPaymentlistEnglish(data: any) {
    ...
    this.exportData = this.data.map(x => {
      ...
      return {
        ...
        'Total': x.amount,
        'Notes': x.note,
        'Payment Methods': x.paymentMethods[0],
        'Status': x.statusName,
      }
    });
  }
  ...
  exportPdf() {
    ...
    if (currentLang == "ar") {
       head = [['الحالة', ' طريقة القبض', ' ملاحظات', 'المجموع', ' سعر الصرف', 'العمله', ' الفرع', ' تاريخ الحركة', ' رقم الحركة', ' نوع الحركة']]
    }
    else {
       head = [['Payment Methods', 'Customer', 'Notes', 'Total', 'Exchange Rate', 'Currency', 'Branch', ' Transaction Date', 'Transaction Number', 'Transaction Type']]
    }
