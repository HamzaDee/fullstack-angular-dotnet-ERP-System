...
@Component({
  selector: 'app-processoutgoingcheq-list',
  templateUrl: './processoutgoingcheq-list.component.html',
  styleUrls: ['./processoutgoingcheq-list.component.scss']
})
export class ProcessoutgoingcheqListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.SetTitlePage();
    this.GetProcessingOutgoingChequeList();
    this.getFavouriteStatus(this.screenId);
  }

  SetTitlePage() {
    this.TitlePage = this.translateService.instant('ProcoutcheqList');
    this.title.setTitle(this.TitlePage);
  }

  formatCurrency(value: number, decimalPlaces: number): string {

    return this.procOutcheqService.formatCurrency(value, decimalPlaces);
  }

  GetProcessingOutgoingChequeList() {
    var currentLang = this.jwtAuth.getLang();
    const isArabic = currentLang === 'ar';

    this.showLoader = true;
    setTimeout(() => {
      this.procOutcheqService.GetProcessingOutgoingChequeList().subscribe(result => {
        debugger
        ...
        if (currentLang == "ar") {
          this.refreshProcessoutgoingcheqArabic(this.tabelData);
        }
        else {
          this.refreshProcessoutgoingcheqEnglish(this.tabelData);
        }
  ...
  PostProcessingOutgoingCheque(id: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.procOutcheqService.PostProcessingOutgoingCheque(id).subscribe((result) => {
          if (result.isSuccess) {
  ...
  DeleteProcessingOutgoingCheque(id: any) {
    Swal.fire({
      title: this.translateService.instant('AreYouSure?'),
  ...
  OpenProcessingOutgoingChequeForm(id: any) {
    this.routePartsService.GuidToEdit = id
    this.routePartsService.Guid2ToEdit = 'Show';
    this.routePartsService.Guid3ToEdit = true;
    this.router.navigate(['ProcessingOutcomingCheque/ProcoutcheqForm']);
  }

  EditProcessingOutgoingChequeForm(id: any) {
    debugger
    this.routePartsService.GuidToEdit = id
  ...
  AddProcessingOutgoingChequeForm(id: any) {
    debugger
    this.routePartsService.GuidToEdit = id
  ...
  CopyProcessingOutgoingCheque(id: any) {
    this.routePartsService.GuidToEdit = id;
  ...
  ReverseProcessingOutgoingCheque(id: any) {
    this.routePartsService.GuidToEdit = id;
  ...
  AttachmentProcessingOutgoingCheque(id: any) {
    this.routePartsService.GuidToEdit = id;
    debugger
  ...
  exportPdf() {
    ...
    if (currentLang == "ar") {
      footRow[5] = "المجموع";
      footRow[6] = this.formatCurrency(totalAmount, 3); foot = [footRow.reverse()];
    }
    else {
      footRow[5] = "Total";
    ...
    const title = currentLang === "ar"
      ? "كشف قائمة معالجة الشيكات الصادرة"
      : "List Of Processing Outcoming Cheque";
  ...
  refreshProcessoutgoingcheqArabic(data) {
  ...
  refreshProcessoutgoingcheqEnglish(data) {
    this.exportData = data;
    this.exportData = this.tabelData.map(x => {
