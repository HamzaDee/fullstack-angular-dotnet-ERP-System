...
@Component({
  selector: 'app-returnsalesinvoice-list',
  templateUrl: './returnsalesinvoice-list.component.html',
  styleUrls: ['./returnsalesinvoice-list.component.scss']
})
export class ReturnsalesinvoiceListComponent implements OnInit {
...
constructor(
  ...
  {
    this.SetTitlePage();
    this.GetReturnSalesInvoiceList();
    this.getFavouriteStatus(this.screenId);
  }

  SetTitlePage() {
    this.TitlePage = this.translateService.instant('ReturnSalesInvoiceList');
  ...
  GetReturnSalesInvoiceList() {
    ...
    setTimeout(() => {
      this.returnService.GetReturnSalesInvoiceList(this.voucherTypeEnum).subscribe(result => {
        debugger
        ...
        if(currentLang == "ar"){
          this.refresReturnsalesinvoiceArabic(this.tabelData);
         }
         else{
          this.refreshReturnsalesinvoiceEnglish(this.tabelData);
         }   
...
if(result.length > 0 )
{
  ...
  PostReturnSalesInvoice(Id,voucherTypeId,voucherNo) {
    Swal.fire({
      ...
      if (result.value) {
        this.returnService.PostReturnSalesInvoice(Id,voucherTypeId,voucherNo).subscribe((result) => {
          if (result.isSuccess) {
  ...
  DeleteReturnSalesInvoice(Id,voucherTypeId,voucherNo) {
    debugger
    Swal.fire({
      title: this.translateService.instant('AreYouSure?'),
      ...
      if (result.value) {
        this.returnService.DeleteReturnSalesInvoice(Id,voucherTypeId,voucherNo,this.voucherTypeEnum).subscribe((result) => {
          debugger
          if (result.isSuccess) {
  ...
  ShowDetailsOnly(id: any) {
    ...
    this.router.navigate(['ReturnSalesInvoice/ReturnSalesInvoiceForm']);
  }

  AddNewReturnSalesInvoice(id: any) {
    this.routePartsService.GuidToEdit = id
  ...
  AttachmentReturnSalesInvoice(id: any) {
    this.routePartsService.GuidToEdit = id;
    debugger      
  ...
  refresReturnsalesinvoiceArabic(data) {
    debugger
    this.Data = data;
  ...
  refreshReturnsalesinvoiceEnglish(data) {
    debugger
    this.Data = data;
    this.exportData = this.Data.map(x => {
  ...
  exportPdf() {
    ...
    if(currentLang == "ar"){
       head = [['الحالة','القيمة',' ملاحظات',' المستودع ','العميل',' الفرع',' تاريخ السند',' نوع السند',' رقم السند']]    }
    else{
       head = [['Status','Value','Notes','Store','Client','Branch',' Voucher Date','Voucher Type','Voucher Number']]
    }
    ...
    const Title = currentLang == "ar" ? " قائمة مردودات المبيعات": "Return Sales Invoice List";
