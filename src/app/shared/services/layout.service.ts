...
//   public fullWidthRoutes = ['shop'];
  public fullWidthRoutes = ['Emergency','Patient','dialysis','Laboratory','Patient','Supplies'];
  public SidebarStyle  : string ='full';
  ...
  publishLayoutChange(lc: ILayoutConf, opt: ILayoutChangeOptions = {}) {
    this.SidebarStyle = this.isMobile? this.SidebarStyle  : lc.sidebarStyle ?? this.SidebarStyle 
    ...
    if (this.currentRoute) {
      this.fullWidthRoutes.forEach((route) => {
        if (this.currentRoute.indexOf(route) !== -1) {
          // sidebarStyle = 'closed';
          sidebarStyle = this.isMobile ?sidebarStyle: this.SidebarStyle ?? 'full';
          this.layoutConf.sidebarStyle =sidebarStyle
        }
      });
    }
